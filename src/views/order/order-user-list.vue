...
<script>
    ...
    import {batchExport, getAmountInfo, merchantTradeList} from "../../api/trade";
    ...
    export default {
        ...
        methods: {
            ...
            getList: async function (cp, c) {
                ...
                const result = await merchantTradeList(query)
                if (result.code == 20000) {
                    this.list = result.data.content;
                    this.page.total = result.data.totalElements
                    if (this.list.length > 0 && !this.list[0].showRebate) {
                        this.columns = [
                            {
                                title: '序号',
                                type: 'index',
